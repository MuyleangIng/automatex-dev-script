...
export default function AppTable({ appType }) {
  const { data, isLoading, error } = useFindAppsQuery(
    `?page=1&limit=1000&appType=${appType}`
  );
  ...
  if (isLoading) {
    return <AppTableSkeleton appType={appType} />;
  }
...
function AppTableRow({ no, app, apps, setApps }) {
  let domainName;

  if (!app.domains[0]?.subdomain) {
    domainName = "unknown";
  } else {
    domainName = app.domains[0]?.subdomain + ".automatex.dev";
    domainName = (
      <a
