...
import {
    selectDeploymentApp, selectError, selectIsLoading
...
function GitAutomateX({params}) {
    ...
    const command2 = [`git remote add origin ${data?.repo?.http_url_to_repo || 'default_value'} \n`, 'git branch -M main\n', 'git push -u origin main']
    ...
    >
        <>
            ...
            {gitProjects?.empty_repo === false ? (<div
                            ...
                            <Card>
                                <Table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600 text-2xl">
                                    <Table.Head className="bg-gray-100 dark:bg-gray-700">
                                        ...
                                        <Table.HeadCell className="mr-4 items-end">
                                            <div
                                                className={'text-sm font-medium'}>Branch: {gitProjects?.default_branch}</div>
                                        </Table.HeadCell>
                                        <Table.HeadCell>
                                            <Button as={Link} href={gitProjects?.http_url_to_repo}  target={"_blank"} className={" w-52 mx-auto "}>Go to Resource</Button>
                                        </Table.HeadCell>
                                    </Table.Head>
                                    <Table.Body
                                        className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                        {projectTree?.map((file, index) => (<Table.Row key={index}>
                                                <Table.Cell className={'flex items-center gap-2'}>
                                                    {file.type === 'blob' ? (<>
                                                            <CiFileOn/>{file.name}</>) : file.type === 'tree' ? (<>
                                                            <FaFolder/>{file.name}</>) : null}
                                                </Table.Cell>
                                            </Table.Row>))}
                                    </Table.Body>
                                </Table>
                    ...
                    <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                        <div className="sm:col-span-2">
                            ...
                            <Card className={"m-5"}>
                                ...
                                <div className="flex items-center text-2xl font-bold dark:text-white">
                                    <div className="flowbite-container">
                                        <span className={"text-cyan-500"}>(</span> push an existing repository from the
                                        command line
                                        line<span className={"text-cyan-500 "}> )</span>
                                    </div>
                                </div>
